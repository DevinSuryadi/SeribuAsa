...
export default function AdminReportsPage() {
      ...
      {items.length === 0 ? (
        ...
        <div className="space-y-2">
          {items.map((item) => (
            <div
              key={item.label}
              className="rounded-xl bg-white px-3 py-2 flex items-center justify-between gap-3"
            >
              <div>
  ...
  return (
    <DashboardLayout
      ...
      subtitle="Ringkasan admin, regional report, dan demografi dari backend yang sudah terhubung."
    >
          ...
          <>
              ...
              {summaryCards.map((card) => {
                ...
                return (
                  <div key={card.label} className={`rounded-2xl border p-5 ${card.border} ${card.bg}`}>
                    <div
                      className={`flex h-9 w-9 items-center justify-center rounded-lg border bg-white ${card.border} mb-3`}
                    >
            ...
            <div className="grid gap-4 lg:grid-cols-2">
                  ...
                  <>
                    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                      <div className="rounded-xl bg-slate-50 p-4">
                        ...
                        <div className="text-lg font-semibold text-foreground">
                          {regionalReport.coverage.total_beneficiaries}
                        </div>
                      </div>
                      <div className="rounded-xl bg-slate-50 p-4">
                        <div className="text-sm text-muted-foreground">Children</div>
                        <div className="text-lg font-semibold text-foreground">
                          {regionalReport.coverage.total_children}
                        </div>
                      </div>
                      <div className="rounded-xl bg-slate-50 p-4">
                        <div className="text-sm text-muted-foreground">Vendors</div>
                        <div className="text-lg font-semibold text-foreground">
                          {regionalReport.coverage.total_vendors}
                        </div>
                      </div>
                      <div className="rounded-xl bg-slate-50 p-4">
                        <div className="text-sm text-muted-foreground">Districts</div>
                        <div className="text-lg font-semibold text-foreground">
                          {regionalReport.coverage.districts_covered}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                      <div className="rounded-xl bg-slate-50 p-4">
                        ...
                        <div className="text-lg font-semibold text-foreground">
                          {regionalReport.stunting_rate.current.toFixed(1)}%
                        </div>
                        <div className="text-xs text-muted-foreground">
                          Sebelumnya {regionalReport.stunting_rate.previous.toFixed(1)}% ·{" "}
                          {regionalReport.stunting_rate.trend}
                        </div>
                      </div>
                      <div className="rounded-xl bg-slate-50 p-4">
                        <div className="text-sm text-muted-foreground">Budget utilization</div>
                        <div className="text-lg font-semibold text-foreground">
                          {regionalReport.budget_utilization.percentage.toFixed(1)}%
                        </div>
                        <div className="text-xs text-muted-foreground">
                          {formatIDR(regionalReport.budget_utilization.utilized)} dipakai dari{" "}
                          {formatIDR(regionalReport.budget_utilization.allocated)}
                        </div>
                      </div>
                      <div className="rounded-xl bg-slate-50 p-4">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {formatReportDate(regionalReport.period?.end_date)}
                        </div>
                      </div>
                    </div>
                        ...
                        <tbody className="divide-y divide-border">
                          {regionalReport.district_breakdown.map((district) => (
                            <tr key={district.district}>
                              ...
                              <td className="px-4 py-3 text-muted-foreground">
                                {district.stunting_rate.toFixed(1)}%
                              </td>
                            </tr>
                          ))}
                        </tbody>
              ...
              <div className="rounded-2xl border border-border bg-card p-5 space-y-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Distribusi data dari backend admin reports.
                  </p>
                </div>
                ...
                {demographicsReport ? (
                  <div className="grid gap-3">
                    ...
                    {renderDistributionList(
                      "Gender distribution",
                      demographicsReport.gender_distribution
                    )}
                    {renderDistributionList(
                      "Nutrition status",
                      demographicsReport.nutrition_status
                    )}
                    {renderDistributionList(
                      "FIES classification",
                      demographicsReport.fies_classification
                    )}
                  </div>
                ) : (
                  <div className="rounded-2xl border border-border bg-slate-50 p-4 text-sm text-muted-foreground">
            ...
            <div className="rounded-2xl border border-border bg-card p-5">
              ...
              <p className="text-sm text-muted-foreground mb-4">
                Gunakan endpoint backend admin yang sudah siap untuk download CSV.
              </p>
              <div className="flex flex-wrap gap-2">
                {reports.map((report) => (
                  <Button
                    key={report.filename}
                    variant="outline"
                    onClick={() => void handleExport(report.endpoint, report.filename)}
                  >
