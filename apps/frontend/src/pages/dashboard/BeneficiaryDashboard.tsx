...
export default function BeneficiaryDashboard() {
  ...
  const [fiesStatus, setFiesStatus] = useState<any>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [balanceData, historyData, fiestData, nutritionMeasure] = await Promise.all([
  ...
  return (
    <DashboardLayout
      title="Beranda Penerima Manfaat"
          ...
          <Card>
            <CardContent className="pt-6">
              ...
              <div className="text-2xl font-bold text-foreground">
                {fiesStatus?.classification || "Belum"}
              </div>
                  ...
                  {transactions.slice(0, 4).map((t: any) => {
                    ...
                    return (
                      <div key={t.id} className="flex items-center gap-3">
                        <div
                          className={`flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full ${isCredit ? "bg-primary/10" : "bg-secondary"}`}
                        >
                          <Wallet
                            className={`h-4 w-4 ${isCredit ? "text-primary" : "text-muted-foreground"}`}
                          />
                        </div>
                        <div className="flex-1 min-w-0">
                        ...
                        <div
                          className={`text-sm font-semibold flex-shrink-0 ${isCredit ? "text-primary" : "text-foreground"}`}
                        >
                          {isCredit ? "+" : "-"}
