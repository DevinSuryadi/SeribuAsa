...
const PemantauanGizi = () => {
  ...
  const getStatusColor = (classification: string) => {
    switch (classification) {
      ...
      case "moderate_malnourished":
        return "bg-accent/10 text-accent-foreground border-accent/20";
      case "severe_malnourished":
  ...
  if (loading) {
    return (
      <DashboardLayout
        ...
        subtitle="Pantau tumbuh kembang dan status gizi anak Anda."
      >
          ...
          {children.length === 0 ? (
            <Card className="md:col-span-2 text-center">
              <CardContent className="pt-8 pb-8">
                ...
                <p className="text-muted-foreground">Belum ada data anak</p>
            ...
            children.map((child) => {
              ...
              return (
                <Card
                  key={child.id}
                  className={`cursor-pointer hover:border-primary/30 transition-colors ${selectedChild?.id === child.id ? "border-primary/30" : ""}`}
                ...
                >
                  <CardContent className="pt-6">
                    <div className="flex items-center gap-4">
                      ...
                      {latest ? (
                        <Badge variant="outline" className={getStatusColor(latest.classification)}>
                          {getStatusLabel(latest.classification)}
                        </Badge>
                      ) : (
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <Activity className="h-5 w-5 text-primary" /> Grafik Tumbuh Kembang -{" "}
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <Input
                  type="number"
                  value={formMuac}
                  onChange={(e) => setFormMuac(e.target.value)}
                  placeholder="0.0"
                  step="0.1"
                />
              </div>
