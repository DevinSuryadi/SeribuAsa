...
import { ClipboardList, ChevronRight, ChevronLeft, CheckCircle, AlertTriangle, Info } from 'lucide-react';
...
const SurveiFIES = () => {
  ...
  const getSeverity = (s: number) => {
    if (s <= 2) return { label: 'Rendah', color: 'bg-primary/10 text-primary border-primary/20', desc: 'Ketahanan pangan Anda dalam kondisi baik.' };
    if (s <= 5) return { label: 'Sedang', color: 'bg-accent/10 text-accent-foreground border-accent/20', desc: 'Ada indikasi kerawanan pangan sedang. Anda akan mendapat prioritas bantuan.' };
    return { label: 'Parah', color: 'bg-destructive/10 text-destructive border-destructive/20', desc: 'Terdeteksi kerawanan pangan serius. Bantuan Anda akan diprioritaskan segera.' };
  };
      ...
      <DashboardLayout title="Survei FIES" subtitle="Survei ketahanan pangan bulanan.">
        <div className="max-w-lg mx-auto space-y-6">
            ...
            <CardContent className="pt-8 pb-8 space-y-6">
              ...
              <div className="rounded-lg bg-secondary/50 p-4 text-left space-y-2">
                ...
                <ul className="text-xs text-muted-foreground space-y-1 ml-6">
                  ...
                  <li>• Wajib diisi setiap bulan</li>
          ...
          <Card>
            ...
            <CardContent>
              {[
                { month: 'Januari 2026', score: 3, severity: 'Sedang' },
                { month: 'Desember 2025', score: 4, severity: 'Sedang' },
                { month: 'November 2025', score: 5, severity: 'Sedang' },
              ].map((h) => (
                <div key={h.month} className="flex items-center justify-between border-b border-border/50 py-2 last:border-0">
                  <span className="text-sm text-muted-foreground">{h.month}</span>
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-medium text-foreground">{h.score}/8</span>
                    <Badge variant="outline" className={getSeverity(h.score).color}>{h.severity}</Badge>
                  </div>
                </div>
              ))}
            </CardContent>
          ...
          <CardContent className="pt-8 pb-8 space-y-8">
            ...
            <div className="flex items-center justify-between">
              ...
              <Button onClick={handleNext} className="gap-1">
                {currentQ === fiesQuestions.length - 1 ? 'Selesai' : 'Selanjutnya'} <ChevronRight className="h-4 w-4" />
              </Button>
            </div>
          </CardContent>
        </Card>
