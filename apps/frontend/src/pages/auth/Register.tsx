...
import { Shield, Eye, EyeOff, Heart, Users, Store, Loader2, CheckCircle2, Circle } from "lucide-react"
    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-100 px-4">
      <div className="w-full max-w-md bg-white rounded-xl shadow-lg p-8">
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginBottom: 28 }}>
          ...
          <h1 style={{ fontSize: 24, fontWeight: 700, marginTop: 14, color: "#111", letterSpacing: "-0.02em" }}>
            SeribuAsa
          </h1>
        </div>

        <h2 style={{ fontSize: 22, fontWeight: 600, textAlign: "center", marginBottom: 4, color: "#111" }}>
          Buat Akun Baru
        </h2>
        <p style={{ fontSize: 14, color: "#737373", textAlign: "center", marginBottom: 28 }}>
          Bergabung dengan gerakan nutrisi anak Indonesia
        </p>

        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          <div>
            <label style={{ fontSize: 13, fontWeight: 500, color: "#404040", display: "block", marginBottom: 10 }}>
              Daftar sebagai
            </label>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
              {roles.map((r) => (
                ...
                >
                  <r.icon size={22} style={{ color: selectedRole === r.id ? "#16a34a" : "#a3a3a3" }} />
                  <span style={{ fontSize: 12, fontWeight: 600, color: selectedRole === r.id ? "#16a34a" : "#525252" }}>
                    {r.label}
                  </span>
                  <span style={{ fontSize: 10, color: "#a3a3a3", textAlign: "center" }}>{r.desc}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            {password.length > 0 && (
              <div style={{ marginTop: 10, padding: "12px 14px", background: "#fafafa", borderRadius: 10 }}>
                  ...
                  {passwordRequirements.map((req, idx) => {
                    ...
                    return (
                      <div key={idx} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                        {valid ? <CheckCircle2 size={12} style={{ color: "#16a34a" }} /> : <Circle size={12} style={{ color: "#d4d4d4" }} />}
                        <span style={{ fontSize: 11, color: valid ? "#16a34a" : "#a3a3a3" }}>{req.label}</span>
                      </div>
                    )
                  })}
                </div>
            ...
            <div style={{ position: "relative" }}>
              <input
                ...
                style={{
                  ...
                  padding: "12px 40px 12px 14px",
                  borderRadius: "12px",
                  border: confirmPassword.length > 0 ? (passwordsMatch ? "1.5px solid #16a34a" : "1.5px solid #ef4444") : "1.5px solid #e5e5e5",
                  fontSize: 14,
                  outline: "none",
                  boxSizing: "border-box",
                  transition: "border-color 0.15s ease",
          ...
          <button
            type="submit"
            disabled={loading || !selectedRole || !fullName.trim() || !email || !passwordValid || !passwordsMatch}
            style={{
              width: "100%",
              padding: "14px",
              borderRadius: "12px",
        ...
        <p style={{ fontSize: 13, textAlign: "center", color: "#737373", marginTop: 24 }}>
          Sudah punya akun?{" "}
          <Link to="/masuk" style={{ color: "#16a34a", fontWeight: 600, textDecoration: "none" }}>
            Masuk
          </Link>
        </p>
      </div>
